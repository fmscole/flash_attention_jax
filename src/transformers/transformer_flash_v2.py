"""
Transformer 基础模块 — 基于 FlashAttention-2 (flash_attention_v2)

提供可复用的 Transformer 构建块，所有层遵循 stax (init_fun, apply_fun) 约定，
可被其他模块（如 ViT、翻译模型等）直接导入使用。

与 v1 (transformer_flash_v1.py) 的区别：
    本模块使用 flash_attention_v2（FlashAttention-2），其反向传播采用
    KV-outer / Q-inner 的循环顺序（对应 FA2 论文 §3.2 的序列长度并行化），
    而 v1 使用 Q-outer / KV-inner。前向传播结果在数值上完全一致。

导出接口:
    MultiHeadSelfAttention   — 多头自注意力（FlashAttention-2）
    MultiHeadCrossAttention  — 多头交叉注意力（FlashAttention-2）
...
from flash_attention.flash_attention_v2 import flash_attention_v2
...
def _safe_block_size(requested, seq_len):
    ...
    flash_attention_v2 要求 query/key seq_len 能被 block_size 整除。
...
def _broadcast_padding_mask(mask, n_heads):
    ...
    flash_attention_v2 期望 mask 的 batch 维度已合并 heads。
...
def MultiHeadSelfAttention(n_heads, head_dim, causal=False,
    ...
    def apply_fun(params, inputs, padding_mask=None, **kwargs):
        ...
        # 合并 batch 和 heads：flash_attention_v2 期望 (batch*n_heads, seq, head_dim)
        ...
        # FlashAttention-2 — 自动 clamp block_size 到能整除 seq_len 的安全值
        bq = _safe_block_size(block_size_q, seq_len)
        bk = _safe_block_size(block_size_kv, seq_len)
        attn_out = flash_attention_v2(
            Q, K, V,
            padding_mask_k=mask,
            padding_mask_q=mask,
            causal=causal,
...
# ═══════════════════════════════════════════════════════════════════
# 2. 多头交叉注意力 — FlashAttention-2
# ═══════════════════════════════════════════════════════════════════

def MultiHeadCrossAttention(n_heads, head_dim,
                            block_size_q=1024, block_size_kv=1024):
    """多头交叉注意力层（FlashAttention-2），遵循 stax 约定。
...
def TransformerEncoderBlock(n_heads, head_dim, embed_dim, mlp_dim,
    ...
    """Pre-LN Transformer 编码器块（FlashAttention-2）。
...
def TransformerDecoderBlock(n_heads, head_dim, embed_dim, mlp_dim,
                            dropout_rate=0.1, block_size=1024):
    """Pre-LN Transformer 解码器块（FlashAttention-2）。
...
def Transformer(src_vocab_size, tgt_vocab_size,
    ...
    """完整的 Encoder-Decoder Transformer 模型（FlashAttention-2）。
...
def _split_rng(rng):
    """安全拆分 rng，兼容 rng=None（eval 模式 dropout 自动关闭）。"""
    if rng is not None:
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("FlashAttention-2 Transformer — 模块测试")
    print("=" * 60)

    # ---- 测试 Encoder ----
    print("\n[1] 测试 TransformerEncoder (FA2)...")
    ...
    # ---- 测试自注意力 (ViT 风格) ----
    print("\n[4] 测试 MultiHeadSelfAttention (FA2, ViT 风格)...")
    ...
    # 测试带 padding_mask
    print("\n[5] 测试 MultiHeadSelfAttention + padding_mask (FA2)...")
    ...
    from transformers.transformer_flash_v1 import (
        MultiHeadSelfAttention as MSA_v1,
    )
    attn_v1_init, attn_v1_apply = MSA_v1(
        n_heads=6, head_dim=64, causal=False,
        block_size_q=65, block_size_kv=65,
    )
    _, attn_v1_params = attn_v1_init(init_rng, vit_shape)
    v1_out = attn_v1_apply(attn_v1_params, vit_x)
    v2_out = attn_apply(attn_params, vit_x)
    diff = jnp.max(jnp.abs(v1_out - v2_out))
    print(f"   V1 vs V2 最大差异: {diff:.8f} {'✓' if diff < 1e-6 else '✗'}")
